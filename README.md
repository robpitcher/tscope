...
- 💰 **Per-session cost** — uses complete shutdown totals and per-model credits when available, while retaining OTel token and trace detail
- 📅 **Recent-by-default** — shows the 20 most recent sessions by default
- 📈 **HTML dashboard** — friendly session names, click-to-copy resume commands, token charts, sort controls, CSV export, and system light/dark theme
...
- 📤 **JSON output** — machine-readable schema (`tscope/report/v8`) for scripting
...
| Mode | Behavior | Cost |
|---|---|---|
| **`--source auto`** (default) | Reads OTel (`~/.copilot/tscope/otel.jsonl`) and log-parser sessions (`~/.copilot/session-state/`) into a **merged report**. On overlap, OTel supplies tokens/traces while the matching shutdown log supplies complete session/per-model credits and exact API time when present. | ✅ Complete shutdown credits with OTel detail |
...
| **`--source logs`** | Reads only log-parser sessions. Shows session/per-model credits and exact API time when shutdown fields are present. | ✅ Shutdown credits (Copilot CLI 1.0+) |
...
| `--json` | _(none)_ | Emit the report as JSON (`tscope/report/v8`) to stdout instead of formatted text. | No |
...
| `--source` | `auto` \| `otel` \| `logs` | Data source. `auto` (default): merges OTel and logs field-by-field on overlap. `otel`: OTel only; exits with error if unavailable. `logs`: log parser only. | No |
...
- [JSON Output](docs/json-output.md) — `tscope/report/v8` schema reference
