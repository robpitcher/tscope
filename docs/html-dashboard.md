...
- **Per-session source badge** — every session card displays its primary source ("OTel" or "log parser"). Metric tooltips identify when cost or API time was enriched from the matching event log.
...
- Per-session cards with:
  ...
  - **API-time chip** (blue) — shown on log sessions and overlapping OTel sessions when exact shutdown `totalApiDurationMs` is available. OTel span wall time is not used as an estimate.
