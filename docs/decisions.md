...
Architecture and design decisions for tscope, newest sections first. Entries carry the
author names used by the AI agent team that produced them; that team framework has since
been removed from the repo, but the decisions themselves still stand. Older entries live
in [decisions-archive.md](decisions-archive.md).

## Active Decisions

## Agentic Workflow Files vs Dependabot (2026-07-25)

**Status:** Active

`.github/workflows/` holds two classes of file:

- **Generated by `gh aw compile`** — `*.lock.yml` and `agentics-maintenance.yml`. Never
  edit these by hand. Their action pins are hardcoded by the gh-aw compiler
  (`pkg/workflow/data/action_pins.json`), and the `aw-compile-check` job fails if they
  drift from compiler output.
- **Hand-written** — `ci.yml`, `verify.yml`, `release.yml`, `aw-compile-check.yml`.

Dependabot used to scan both, which produced permanently-failing PRs: it would bump an
action inside a `.lock.yml`, then `gh aw compile` would revert the bump and the check
would fail on the stale diff (#81, #82, #97, #103). Action pins inside generated files
move **only** by upgrading the gh-aw CLI and recompiling.

**Decision:** `dependabot.yml` now excludes `.github/workflows/*.lock.yml` and
`.github/workflows/agentics-maintenance.yml` via `exclude-paths`. To move those pins,
...
The Squad agent-team framework (`.squad/`, `.github/agents/squad.agent.md`, and the
`squad-triage` / `squad-issue-assign` / `squad-heartbeat` / `sync-squad-labels`
workflows) was removed. It was also the last source of floating action tags
(`actions/checkout@v7`, `actions/github-script@v9`) in the repo.

tscope-specific content was preserved rather than deleted:

- `.squad/decisions.md` → `docs/decisions.md`
- `.squad/decisions-archive.md` → `docs/decisions-archive.md`
- `.squad/skills/{otel-parsing,inline-js-validation,windows-playwright-screenshots}` →
  `.copilot/skills/`
