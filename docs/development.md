...
tscope/
├── src/
│   ├── index.ts              # CLI entry point, subcommand routing
│   ├── args.ts               # Argument parsing, validation, and predicate building
...
│   ├── workspace.ts          # workspace.yaml reader (client_name enrichment)
...
│   │   ├── otelSource.ts     # OTel data source (otel.jsonl + archives)
